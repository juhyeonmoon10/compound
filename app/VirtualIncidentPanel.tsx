...
export default function VirtualIncidentPanel({ settings, onApply, driverLabel, othersCount }: {
  ...
  return <form className="incident-settings" onSubmit={event => {
    ...
    <div className="incident-settings__intro"><span>VIRTUAL RACE ONLY</span><h4>사고도 레이스의 변수로.</h4>
      <p>내 차와 상대 차량의 사고 확률을 직접 정하세요. 실제 기록·추천 전략·기본 전략 점수는 바뀌지 않습니다.</p>
    ...
    <details className="incident-settings__rules"><summary>깃발 효과·가정·출처</summary>
      <dl><dt>옐로우</dt><dd>사고 차량 12초 정차 후 복귀. 30초 동안 해당 가상 섹터 감속·추월 제한.</dd>
      ...
      <p>VSC·SC·레드 사고 차량은 DNF 처리합니다. 피트 작업은 감속 중 정상 속도로 진행하며 레드 중에는 멈춥니다. 레드는 현 위치 동결로 단순화합니다. 실제 피트 집결·무료 타이어 교체·재출발 그리드·차량 충돌 물리·사고 중 타이어 온도 변화는 재현하지 않습니다. SC 차량 3D 모델 대신 대열 제어로 구현합니다.</p>
...
export function VirtualIncidentLog({ race, frame, onSeek, hasStarted = true }: { race: VirtualRace; frame: RaceGridFrame; onSeek: (seconds: number) => void; hasStarted?: boolean }) {
  const events = useMemo(() => {
    ...
    const incidents = race.incidents.flatMap(event => [
      { id: event.id, time: event.startSeconds, label: `${telemetryDriverName(event.driverId, event.label)} · L${event.lap} S${event.sector} · ${FLAG_LABELS[event.flag]} · ${event.retired ? "DNF" : "스핀 후 복귀"}` },
      ...(!event.retired ? [{ id: `${event.id}:recovery`, time: event.startSeconds + INCIDENT_MODEL.recoverySeconds, label: `${event.label} · 스핀 후 주행 복귀` }] : []),
  ...
  return <div className="incident-log">
    <p>가상 사고 적용 기록 · 시드 {race.settings.seed} · 경과 시간에는 레드 중단 시간이 포함됩니다.</p>
    <div className="incident-log__table"><table><caption>가상 레이스 전체 순위</caption><thead><tr><th>순위</th><th>선수</th><th>랩</th><th>상태</th><th>선두 간격</th></tr></thead>
      <tbody>{frame.cars.map(car => <tr key={car.id} data-player={car.id === race.playerId}><td>{car.position}</td>
        ...
        <td>{car.retired ? "DNF" : car.completed ? "완주" : car.incidentStopped ? "사고 · 정차" : car.isPitting ? "PIT" : car.compound}</td>
