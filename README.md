...
`3D 리플레이 → 사고·깃발`에서 활성화합니다. 기본 OFF이며, 선택한 선수와 다른 선수 **각각의 경기당 사고 확률**을 0–100%로 따로 설정합니다. 5% / 3%는 입력 예시이지 실제 사고율이 아닙니다. 각 선수는 독립 추첨하며 경기당 최대 한 건입니다. 자동 대응의 옐로우/VSC/SC/레드 비중 45/25/25/5% 역시 교육용 가정입니다. 특정 깃발 고정은 실험용이며 공식 사고 판정이 아닙니다.
...
- 0.5초 간격으로 한 번 사전 계산한 시계를 3D·속도계·순위·피트 표식·이벤트 기록에서 공유합니다. 타이어 상태는 기존 랩 비용의 추정치이고 사고 정차 중 열·마모 변화는 별도로 계산하지 않습니다. 실제 충돌·손상 물리는 없습니다.
