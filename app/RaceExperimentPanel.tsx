...
export default function RaceExperimentPanel({ candidates, seed, onSeedChange, result, onResult, racecraft, startingGridPosition, pitLossSeconds, trialIndex, onTrialChange, hidden, fixedRivals, playerId, gridSlotOffsetSeconds, eventPrior }: {
  ...
  const run = () => {
    ...
    onResult(null);
    setRunning(true); setStatus("같은 레이스 상황으로 300회를 계산하고 있습니다…");
    try {
      ...
      instance.onmessage = (message: MessageEvent<RaceExperimentWorkerResponse>) => {
        ...
        if (message.data.result) {
          ...
          onResult(next); onTrialChange(0); setStatus("300회 계산을 완료했습니다.");
          finish();
  ...
  return <section className="race-experiment" hidden={hidden} aria-labelledby="race-experiment-title">
    <header><div><span>확률 실험</span><h3 id="race-experiment-title">300회 레이스 비교</h3><p>모든 전략에 같은 SC·VSC와 교통 조건을 적용합니다.</p></div>
    ...
    <details><summary>모델 가정</summary><p>과거 관측 빈도를 사용하더라도 실제 경기의 발생 시점을 예측하는 것은 아닙니다. 한 시행에 SC·VSC 각각 최대 한 구간을 생성하며, 서로 겹치지 않는 자리가 없으면 뒤 구간은 생략됩니다. SC에서는 정상 피트 손실의 {Math.round(MODEL_PARAMS.race.scPitFactor * 100)}%, VSC에서는 {Math.round(MODEL_PARAMS.race.vscPitFactor * 100)}%를 적용합니다.</p><p>앞차와 {MODEL_PARAMS.race.followingGapSeconds}초 이내에서 페이스 차의 {MODEL_PARAMS.race.trafficLossFraction * 100}%를 손실로 적용합니다. {fixedRivals ? "19대 상대와 전략 배정은 3D 레이스와 동일합니다." : "19대 상대는 일반 고정 전략 풀을 사용합니다."} SC의 실제 대열 압축과 피트 운영은 재현하지 않습니다.</p>{result?.limitations.map(note => <p key={note}>{note}</p>)}</details>
  </section>;
}
